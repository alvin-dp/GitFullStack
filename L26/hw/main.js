...
function handleButtonClick(buttonId, message) {
  ...
  elementButton.addEventListener('click',()=> {console.log(message)});
}
 ...
 *
 */
let elementLogger = document.getElementById("mouseLoger");  
function logMouseMove(event) {
  const iText= `Mouse X: ${event.clientX}, Mouse Y: ${event.clientY}`;
  elementLogger.innerText = iText;
  console.log(iText);
}
...
function setupEventDelegation(selector) {
  ...
  parentDiv.addEventListener('click', function(event) {
    if (event.target.tagName === 'LI') {
      let innerText = String(event.target.textContent);
      console.log(`Item clicked: ${innerText.trim()}`);
    }        
  });
}
