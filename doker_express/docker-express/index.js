...
router.get('/listcol', async (req, res) => {
    const collections = await mongoTools.listCollections(process.env.DB_NAME_LOCAL,false);
    const fields = {};
    const doc = await mongoTools.findDocuments(process.env.DB_NAME_LOCAL,'temp',{},fields,false);
...
router.get('/db-crud/:type', async (req, res) => {
    ...
    switch (db_operation) {
        case 'insertOne':
            ...
            query_result  = await mongoTools.insertDocs(process.env.DB_NAME_LOCAL,tempCollection,newCustomer);
        ...
        case 'insertMany':
            ...
            query_result  = await mongoTools.insertDocs(process.env.DB_NAME_LOCAL,tempCollection,newCustomers);
            if (Array.isArray(query_result)) {
              docs  = query_result;
              query_result  = 1;
              }             
        ...
        case 'updateOne':
            ...
            query_result  = await mongoTools.updateOneDoc(process.env.DB_NAME_LOCAL,tempCollection,query,updateFields);
            res.render('dboutput.pug', {db_operation: db_operation, db_result:query_result, data:docs})
            break;
        case 'updateMany':
            query = { email: {$regex: 'john'} };
            updateFields = { phone: '9898989898' };          
            query_result  = await mongoTools.updateDocs(process.env.DB_NAME_LOCAL,tempCollection,query,updateFields);
            res.render('dboutput.pug', {db_operation: db_operation, db_result:query_result, data:docs})
            break;
        case 'replaceOne':
            ...
            query_result  = await mongoTools.replaceOneDoc(process.env.DB_NAME_LOCAL,tempCollection,query,newObj);
            res.render('dboutput.pug', {db_operation: db_operation, db_result:query_result, data:docs})
            break;
        case 'deleteOne':
            query = { phone : { $eq: '202051005' } };
            query_result  = await mongoTools.deleteDoc(process.env.DB_NAME_LOCAL,tempCollection,query);
            res.render('dboutput.pug', {db_operation: db_operation, db_result:query_result, data:docs})
            break;
        case 'deleteMany':
            query = { phone : { $eq: '9898989898' } };
            query_result  = await mongoTools.deleteSomeDocs(process.env.DB_NAME_LOCAL,tempCollection,query);
            res.render('dboutput.pug', {db_operation: db_operation, db_result:query_result, data:docs})
            break;
        case 'findWithProjection':
            const fields = { name: 1, email :1,phone: 1};
            query = {};
            docs = await mongoTools.findDocuments(process.env.DB_NAME_LOCAL,tempCollection,query,fields,false);
            query_result  = 1;
